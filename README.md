...
- Nur bestätigte Ergebnisse aktualisieren Rangliste und Elo. Die vier Elo-Änderungen werden unveränderbar je Partie gespeichert, aber nicht in der Ergebnisübersicht angezeigt.
...
Spieler können zusätzlich gezielt vorbereitet und eingeladen werden. Ein Admin öffnet im eigenen Konto „Spieler-E-Mails“, wählt das Spielerprofil aus und trägt die eindeutige Arbeits-E-Mail ein. Mit „E-Mail zuordnen“ wird die Adresse nur mit dem Spieler verknüpft. „Zuordnen & Einladung vorbereiten“ erzeugt serverseitig einen persönlichen, zeitlich begrenzten Supabase-Link und zeigt im Modal die fertige Nachricht mit Betreff an. Link oder vollständige Nachricht lassen sich kopieren und anschließend über das eigene Mailpostfach verschicken. Der Spieler öffnet den persönlichen Link und legt auf der Webseite sein Passwort selbst fest.
...
Die Migration `20260909200000_player_invitations.sql` und die Edge Function `invite-player` sind in Produktion aktiv. **Allow new users to sign up** bleibt aktiviert; die Begrenzung erfolgt durch Domainliste und Auth Hook. Der Einladungsablauf benötigt keinen externen Maildienst: Die Edge Function erzeugt den Link ausschließlich für einen angemeldeten Admin und versendet selbst keine Nachricht. Die optionale Function-Variable `PUBLIC_SITE_URL` kann die Zieladresse der Einladung überschreiben; ohne sie wird `https://lw-ux.github.io/Padel-Liga/` verwendet.
